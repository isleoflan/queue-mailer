...
namespace IOL\Mailer\v1\Content;

use http\Env;
use IOL\Mailer\v1\DataSource\Database;
use IOL\Mailer\v1\DataSource\Environment;
use IOL\Mailer\v1\DataSource\File;
use IOL\Mailer\v1\DataType\Date;
use IOL\Mailer\v1\DataType\UUID;
...
class Mailer
{
    ...
    public function __construct()
    {
        ...
        do {
            $this->uuid = UUID::v4();
        } while (file_exists(Environment::get('MAIL_PATH') . $this->uuid . '.html'));
    ...
    */
    public function saveHtml(): void
    {
        ...
        file_put_contents(Environment::get('MAIL_PATH') .  $this->uuid . '.html', $mailContent);
    }
    ...
    public function renderMail(): array|string
    {
        ...
        foreach ($this->getAllInlineImages($mailContent) as $image) {
            ...
            try {
                $this->mailer->AddEmbeddedImage(Environment::get('MAIL_PATH') . $imagePath, $imageName);
            } catch (\Exception) {
            }
        }
    ...
    public function setTemplate(string $template): void
    {
        ...
        //$this->setTemplateContentText(file_get_contents(File::getBasePath()' . '/mail/templates/' . $this->getTemplate() . '.txt'));
    }
    ...
    public function addTemplateSetting(string $templateSetting, string $value): void
    {
        $this->templateSettings[$templateSetting] = $value;
    }
    ...
    /**
     * @param string $templateContentHtml
     */
    private function setTemplateContentHtml(string $templateContentHtml): void
    {
        $this->templateContentHtml = $templateContentHtml;
    }

    public function importQueueMail(string $body)
    {
        ...
        foreach(['receiver','subject','attachments','template','variables'] as $testKey){
            if(!isset($body[$testKey])){
                throw new InvalidValueException('Key '.$testKey.' not found');
            }
        }
        ...
        $this->setReceiver($body['receiver']);
        $this->setSubject($body['subject']);
        $this->setTemplate($body['template']);

        foreach($body['variables'] as $key => $value){
            $this->addTemplateSetting($key, $value);
        }
